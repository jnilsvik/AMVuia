...
@WebServlet(name = "VariousToolsServlet", value = "/various-tools")
public class VariousToolsServlet extends HttpServlet {
...
*/
    public ArrayList variousToolsList(PrintWriter out) throws SQLException {
        ArrayList<String> VariousTools = new ArrayList();

        try (
                Connection dbConnection = DBUtils.getNoErrorConnection(out);
                PreparedStatement statement = dbConnection.prepareStatement("SELECT toolName FROM Tool WHERE toolCategory = ('Various Tools')");
                ResultSet resultSet = statement.executeQuery()
        ) {
            while (resultSet.next()) {
                String variousToolsReturn = resultSet.getString("toolName");
                VariousTools.add(variousToolsReturn);
            }
        }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        HeaderFooter.printHeader("Various Tools", out);

        out.println("Hello!");
        out.println("");
        try {
            out.println(variousToolsList(out));
