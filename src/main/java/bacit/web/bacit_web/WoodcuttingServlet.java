...
@WebServlet(name = "WoodcuttingServlet", value = "/woodcutting")
public class WoodcuttingServlet extends HttpServlet {
...
*/
    public ArrayList woodcuttingList(PrintWriter out) throws SQLException {
        ArrayList<String> Woodcutting = new ArrayList();

        try (
                Connection dbConnection = DBUtils.getNoErrorConnection(out);
                PreparedStatement statement = dbConnection.prepareStatement("SELECT toolName FROM Tool WHERE toolCategory = ('Woodcutting')");
                ResultSet resultSet = statement.executeQuery()
        ) {
            while (resultSet.next()) {
                String woodcuttingReturn = resultSet.getString("toolName");
                Woodcutting.add(woodcuttingReturn);
            }
        }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        HeaderFooter.printHeader("Woodcutting", out);

        out.println("Hello!");
        out.println("");
        try {
            out.println(woodcuttingList(out));
